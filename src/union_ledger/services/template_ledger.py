...
Generation writes header metadata cells and fills each month block with evidences.
"""
...
def fill_audit_ledger(
    ...
    *,
    ...
    # Process from bottom to top so row inserts do not invalidate later indices.
    for section in sorted(sections, key=lambda s: s["settlement_row"], reverse=True):
        ...
        header_row = section["header_row"] + 1  # openpyxl is 1-based
